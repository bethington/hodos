...
func (parser *Parser) Parse(calc string) calculation.Calculation {
	calc = strings.TrimSpace(calc)
	if calc == "" {
		return &calculation.ConstantCalculation{Value: 0}
	}
...
func (parser *Parser) parseLevel(level int) calculation.Calculation {
	...
	for {
		...
		node = &calculation.BinaryCalculation{
			Left:  node,
	...
	for {
		...
		node = &calculation.TernaryCalculation{
			Left:   node,
...
func (parser *Parser) parseProduction() calculation.Calculation {
	t := parser.peek()
	...
	case t.Type == lexer.Symbol:
		...
		return &calculation.UnaryCalculation{
			Child: node,
			Op:    op.Function,
		}
...
func (parser *Parser) parseLeafCalculation() calculation.Calculation {
	...
	if t.Type == lexer.Number {
		...
		return &calculation.ConstantCalculation{Value: val}
	}
...
func (parser *Parser) parseFunction(name string) calculation.Calculation {
	...
	return &calculation.BinaryCalculation{
		Left:  firstParam,
		Right: secondParam,
		Op:    function,
	}
...
func (parser *Parser) parseProperty() calculation.Calculation {
	t := parser.peek()
	propType := t.Value
	t = parser.consume()
