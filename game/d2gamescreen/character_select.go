...
func (v *CharacterSelect) loadDeleteCharConfirm() {
	...
	lines := strings.Join(d2util.SplitIntoLinesWithMaxWidth(v.asset.TranslateString(enum.DelCharConfLabel), 29), "\n")
...
func (v *CharacterSelect) createButtons(loading d2screen.LoadingState) {
	...
	v.deleteCharCancelButton = v.uiManager.NewButton(d2ui.ButtonTypeOkCancel,
		v.asset.TranslateString(enum.NoLabel))
	...
	v.deleteCharOkButton = v.uiManager.NewButton(d2ui.ButtonTypeOkCancel, v.asset.TranslateString(enum.YesLabel))
...
func (v *CharacterSelect) updateCharacterBoxes() {
	...
	for i := 0; i < 8; i++ {
		...
		// https://github.com/bethington/Nostos/issues/791
...
func (v *CharacterSelect) OnUnload() error {
	// https://github.com/bethington/Nostos/issues/792
