...
func (v *MainMenu) createMainMenuLabels(loading d2screen.LoadingState) {
	...
	v.versionLabel.SetText("Nostos - " + v.buildInfo.Branch)
	...
	v.copyrightLabel.SetText(v.asset.TranslateString(enum.CopyrightLabel))
	...
	v.copyrightLabel2.SetText(v.asset.TranslateString(enum.AllRightsReservedLabel))
	...
	v.openDiabloLabel.SetText("Nostos is neither developed by, nor endorsed by Blizzard or its parent company Activision")
...
func (v *MainMenu) createMultiplayerLabels() {
	...
	v.tcpIPOptionsLabel.SetText(v.asset.TranslateString(enum.TCPIPOptionsLabel))
	...
	v.machineIP.SetText(v.asset.TranslateString(enum.TCPIPYourIPLabel) + "\n" + v.getLocalIP())
	...
	v.hostTipLabel.SetText(d2ui.ColorTokenize(strings.Join(d2util.SplitIntoLinesWithMaxWidth(
		v.asset.TranslateString(enum.TipHostLabel), 36),
		"\n"), d2ui.ColorTokenGold))
	...
	v.joinTipLabel.SetText(d2ui.ColorTokenize(strings.Join(d2util.SplitIntoLinesWithMaxWidth(
		v.asset.TranslateString(enum.TipJoinLabel), 36),
		"\n"), d2ui.ColorTokenGold))
...
func (v *MainMenu) createLogos(loading d2screen.LoadingState) {
	...
	v.diabloLogoLeft.SetEffect(enum.DrawEffectModulate)
	...
	v.diabloLogoRight.SetEffect(enum.DrawEffectModulate)
...
func (v *MainMenu) createMainMenuButtons(loading d2screen.LoadingState) {
	v.exitDiabloButton = v.uiManager.NewButton(d2ui.ButtonTypeWide, v.asset.TranslateString(enum.ExitGameLabel))
	v.exitDiabloButton.SetPosition(exitDiabloBtnX, exitDiabloBtnY)
	v.exitDiabloButton.OnActivated(func() { v.onExitButtonClicked() })

	v.creditsButton = v.uiManager.NewButton(d2ui.ButtonTypeShort, v.asset.TranslateString(enum.CreditsLabel))
	v.creditsButton.SetPosition(creditBtnX, creditBtnY)
	v.creditsButton.OnActivated(func() { v.onCreditsButtonClicked() })

	v.cinematicsButton = v.uiManager.NewButton(d2ui.ButtonTypeShort, v.asset.TranslateString(enum.CinematicsLabel))
	...
	v.singlePlayerButton = v.uiManager.NewButton(d2ui.ButtonTypeWide, v.asset.TranslateString(enum.SinglePlayerLabel))
	...
	v.btnTCPIPCancel = v.uiManager.NewButton(d2ui.ButtonTypeMedium,
		v.asset.TranslateString(enum.CancelLabel))
	v.btnTCPIPCancel.SetPosition(tcpBtnX, tcpBtnY)
	v.btnTCPIPCancel.OnActivated(func() { v.onTCPIPCancelClicked() })

	v.btnServerIPCancel = v.uiManager.NewButton(d2ui.ButtonTypeOkCancel, v.asset.TranslateString(enum.CancelLabel))
	...
	v.btnServerIPOk = v.uiManager.NewButton(d2ui.ButtonTypeOkCancel, v.asset.TranslateString(enum.OKLabel))
...
func (v *MainMenu) createMultiplayerMenuButtons() {
	v.multiplayerButton = v.uiManager.NewButton(d2ui.ButtonTypeWide,
		v.asset.TranslateString(enum.OtherMultiplayerLabel))
	...
	v.networkTCPIPButton = v.uiManager.NewButton(d2ui.ButtonTypeWide, v.asset.TranslateString(enum.TCPIPGameLabel))
	...
	v.btnTCPIPHostGame = v.uiManager.NewButton(d2ui.ButtonTypeWide, v.asset.TranslateString(enum.TCPIPHostGameLabel))
	...
	v.btnTCPIPJoinGame = v.uiManager.NewButton(d2ui.ButtonTypeWide, v.asset.TranslateString(enum.TCPIPJoinGameLabel))
...
func (v *MainMenu) onGithubButtonClicked() {
	url := "https://www.github.com/Nostos/Nostos"
...
func (v *MainMenu) OnMouseButtonDown(event d2interface.MouseEvent) bool {
	if v.screenMode == ScreenModeTrademark && event.Button() == enum.MouseButtonLeft {
		v.SetScreenMode(ScreenModeMainMenu)
		return true
	}
...
func (v *MainMenu) OnKeyUp(event d2interface.KeyEvent) bool {
	...
	case ScreenModeTrademark: // On retail version of D2, some specific key events (Escape, Space and Enter) puts you onto the main menu.
		switch event.Key() {
		case enum.KeyEscape, enum.KeyEnter, enum.KeySpace:
			v.SetScreenMode(ScreenModeMainMenu)
		}
...
func (v *MainMenu) getLocalIP() string {
	...
	return v.asset.TranslateString(enum.IPNotFoundLabel)
}
