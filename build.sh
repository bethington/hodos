#!/bin/bash
#
# About: Build Nostos automatically
# Author: liberodark
# License: GNU GPLv3

version="0.0.8"
go_version="1.16"
echo "Nostos Build Script $version"
...
go_install() {
	...
	if ! command -v go >/dev/null 2>&1; then
		...
		echo "Install Go for Nostos ($distribution)? y/n"
		read -r choice
...
if [ ! -e "$HOME/.config/Nostos" ]; then
	mkdir -p $HOME/.config/Nostos
fi

if [ -e "$HOME/.config/Nostos/.libs" ]; then
	echo "libraries is installed"
else
	echo "OK" >"$HOME/.config/Nostos/.libs"
	dep_install
fi
...
echo "Build finished. Running Nostos will generate a config.json file."
